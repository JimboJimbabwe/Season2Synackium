...
def format_results_as_text(results):
    ...
    """
    text = []
    text.append("XPATH/XQUERY INJECTION PARAMETER DETECTION RESULTS")
    text.append("=" * 55)
    ...
    if "findings" in results:
        text.append("SPECIAL FINDINGS")
        text.append("-" * 40)
        text.append(f"Type: {results['findings']['type']}")
        text.append(f"Description: {results['findings']['description']}")
        if "content_type" in results["findings"]:
            text.append(f"Content-Type: {results['findings']['content_type']}")
        text.append("")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="XPath/XQuery Injection Parameter Detector Module")
    ...
    if args.xml_file:
        ...
        # Test URL with XPath injection-prone parameters
        test_url = "https://example.com/xml?xpath=/users/user[@id=123]&filter=active"
        ...
        test_request = {
            ...
            "body": """<?xml version="1.0" encoding="UTF-8"?>
                    <request>
                        <query>/users/user[@active='true']</query>
